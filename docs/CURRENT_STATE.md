...
sufficiency-governance material child count = 8
...
- The eight prospective Stage-B measurements now have frozen pre-data
  sufficiency governance at `3f51c4d9...f4ae2c7`, but it is not collection
  authority until an independent exact-hash xHigh review passes. Six finite
  rate minima come from the common uncorrected two-sided 95% Wilson capability
  rule; the exact 1.0 preservation gate carries a transparent point-estimator
  boundary exception because no finite all-success Wilson lower bound can equal
  1; and nearest-rank risk p95 uses 20 observations, the first non-maximum order
  statistic. The outcome-blind monitor requires all eight minima plus complete
  slot accounting and freezes the earliest cutoff. Two inputs remain for the
  later POSTP1-004 implementation: the separately frozen
