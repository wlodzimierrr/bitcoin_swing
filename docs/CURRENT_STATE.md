...
failed ETF calendar in-process authority-boundary hashes =
...
IMPLEMENTATION COMPLETE / FAILED INDEPENDENT EXACT-HASH XHIGH ARCHITECTURE RE-REVIEW

POSTP1-002V2A-AD1-R1 review result =
FAIL — TRANSITIVE REPLAY CLOSURE INVALID

POSTP1-001V2A-AD1-R2 implementation commit =
...
POSTP1-001V2A-PAD1 implementation commit =
...
FROZEN_PRE_DATA_FAILED_INDEPENDENT_EXACT_HASH_FINAL_XHIGH_REVIEW
...
IMPLEMENTATION COMPLETE / FAILED INDEPENDENT EXACT-HASH FINAL XHIGH REVIEW
...
POSTP1-001V2A-PAD2 decision commit =
...
material child contracts bound by the runtime owner attestation = 16

runtime owner-attestation proof strategy =
COMPILED_ROOT_WITNESS_PLUS_CLOSED_AST_STORE_USE_GRAMMAR_PLUS_RUNTIME_SCIENTIFIC_EXECUTION_EPOCH_ATTESTATION
...
NO (abandoned by PAD4 and not reopened by R1; PAD3's 25 functions / 7 classes /
15 values / 14 imports is archived diagnostic evidence only)

scientific execution boundary =
FRESH EXEC'D CPYTHON 3.12.14 PROCESS UNDER -I -S -B PLUS A FRESH EMPTY
PER-WORKER -X pycache_prefix NAMESPACE, ONE REQUEST PER PROCESS, NO SHARED
MUTABLE PYTHON STATE WITH THE APPLICATION PROCESS

executed bytecode binding =
FRESH CONTROLLER-CREATED EMPTY PER-WORKER BYTECODE-CACHE NAMESPACE; repository
and site-packages __pycache__ are never consulted, the worker refuses unless
sys.pycache_prefix is the declared namespace, sys.dont_write_bytecode is True
and the namespace is empty, and every loaded module's __cached__ must lie
inside it; a sourceless or wrong-origin module still refuses on the exact
__file__/__spec__.origin source-path check

pre-I2 certified worker project source fixture =
ETF_CALENDAR_WORKER_PRE_I2_SOURCE_MANIFEST_FIXTURE_V1; the exact reviewed
116-module universe at digest
674b006ae66b8aace3458cb870f898ecad33e1f23833436954d36749b3aadbb8

pre-I2 fixture role =
PRE_I2_CONFORMANCE_FIXTURE_AND_PROVENANCE — architecture conformance test
...
changes and parent-bind it to ETF_PUBLICATION_CALENDAR_AUTHORITY_V1

runtime source-manifest re-derivation permitted =
NO (derivation from live source is authority construction/refreeze only;
runtime consumes an already-frozen expected manifest)

trusted controller authority context =
REQUIRED; a request cannot construct it, a response cannot construct it, live
disk cannot silently replace it and the installed environment cannot silently
replace it; admission requires request==context AND response==context AND
response==request

candidate self-hash =
NOT SELF-EMBEDDED; the review/controller context binds the recomputed parent
hash externally, and POSTP1-001V2A-I2 must bind the certified R1 hash into the
final calendar authority after a review PASS

frozen reviewed third-party semantic/artifact authority =
alembic 1.19.1, cryptography 50.0.1, numpy 2.5.2, scipy 1.18.1,
SQLAlchemy 2.0.52; exact reviewed RECORD digest and normalized RECORD content
manifest digest are parent-bound, install location and absolute RECORD path
remain launch-bound, and every installed file byte RECORD declares a hash for
is verified before any dependency code executes
...
POSTP1-002V2A-PAD4-R1 review result =
FAIL — WORKER BOOTSTRAP SOURCE NOT BOUND BEFORE EXECUTION

POSTP1-002V2A-PAD4-R1 independent reproduction =
candidate parent 3415765f...fde37ebc reproduced exactly; 20/20 material
children reproduce and 20/20 are parent-bound

POSTP1-002V2A-PAD4-R1 blocking invariant =
already-trusted code does not establish the certified identity of the worker
bootstrap source before that source executes

worker bootstrap package surface =
etf_calendar_worker/entry_r1.py, etf_calendar_worker/__init__.py and
etf_calendar_worker/protocol_r1.py, which execute before the normal scientific
verifier is trustworthy

stable pre-launch entrypoint drift =
MARKER PRESENT; FABRICATED SCIENTIFIC RESULT ADMITTED

stable pre-launch protocol drift =
MARKER PRESENT; FABRICATED SCIENTIFIC RESULT ADMITTED

POSTP1-002V2A-PAD4-R1 preserved valid portions =
FRESH-EXEC ISOLATION VALID; REPAIR A BYTECODE BINDING VALID; REPAIR C TRUSTED
CONTROLLER AUTHORITY VALID FOR ITS STATED SCOPE but unable to close bootstrap
ordering because worker-visible authority fields are derivable from the
supplied request; REPAIR D THIRD-PARTY SEMANTIC/CONTENT AUTHORITY VALID;
...
POSTP1-002V2A-PAD4-R2 review result =
FAIL — AUTHORITATIVE LAUNCH / ADMISSION BYPASS

POSTP1-002V2A-PAD4-R2 execution classification =
ETF_CALENDAR_ISOLATED_SCIENTIFIC_WORKER_V1_R2_REQUIRES_FIX

POSTP1-002V2A-PAD4-R2 independent reproduction =
candidate parent 68e6bd07...e52561 reproduced exactly; 22/22 material children
reproduce and 22/22 are parent-bound; restore_artifacts reproduces the whole
namespace byte-exact

POSTP1-002V2A-PAD4-R2 blocking invariant =
scientific admission does not require, and cannot authenticate, the controller
bootstrap pre-verification it attests to; admit_worker_result reads only
exit_status, request_digest, stdout and timed_out, never
outcome.bootstrap_pre_verification

POSTP1-002V2A-PAD4-R2 reproduced bypasses =
(1) _spawn_unverified_worker_process on a drifted bootstrap tree, then
admit_worker_result, admits a fabricated scientific result with
bootstrap_pre_verification None; (2) a caller-fabricated PASS
bootstrap_pre_verification mapping is admitted and scientific_response_evidence
then attests verified_by ALREADY_TRUSTED_CONTROLLER_CODE,
verified_before_subprocess_creation true and the exact trusted bootstrap digest
1811e04d...ead411 when no pre-verification ran; (3) a hand-built
WorkerProcessOutcome with no subprocess ever created is admitted, and a
hand-built AdmissionOutcome is accepted by scientific_response_evidence — this
third path uses only public names and no private helper

POSTP1-002V2A-PAD4-R2 closure analysis =
neither PASS condition holds: admission does not independently require
controller-generated bootstrap pre-verification evidence, and no mechanically
closed call graph or interface boundary prevents a project-owned caller from
combining the raw helper with admission; the only boundary is a leading
underscore, a docstring and a test asserting both; WorkerProcessOutcome and
AdmissionOutcome are public frozen dataclasses with no construction guard and
admit_worker_result performs no isinstance check

POSTP1-002V2A-PAD4-R2 contradicted frozen claims =
...
POSTP1-002V2A-PAD4-R2 corrected portions independently reproduced as valid =
the PAD4-R1 authority-ordering defect IS closed on the authoritative path: all
four bootstrap members, drifted plain and drifted fabricating, refuse before
spawn with 0 subprocesses, 0 pycache allocations, markers ABSENT and nothing
admitted; the four-file bootstrap set is mechanically derived and matches; all
18 module-time executable constructs are covered and uncalled function bodies
are correctly ignored; sys.path shadow, symlink, alternate root, alternate
entrypoint, sourceless and module/package collision all refuse before spawn

POSTP1-002V2A-PAD4-R2 preserved repairs independently reproduced as valid =
FRESH_EXEC_ISOLATION; REPAIR A BYTECODE BINDING; REPAIR B FROZEN SOURCE
AUTHORITY; REPAIR C TRUSTED CONTROLLER AUTHORITY CONTEXT; REPAIR D THIRD-PARTY
SEMANTIC/INSTALLED-CONTENT AUTHORITY; COMPILED ROOT WITNESS; CLOSED STORE
GRAMMAR; EXACT ELEVEN-OWNER GRAPH; DIRECT DEPENDENCY BODY RULE

POSTP1-002V2A-PAD4-R2 secondary candidates =
candidate_review_authority_context accepting any well-formed SHA was
independently re-evaluated and CONFIRMED NOT A DEFECT: the controller modules
read no environment variable, and no scientific request, worker or environment
can select the bound hash

POSTP1-002V2A-PAD4-R2 failure classification =
API/ADMISSION BYPASS, not a bootstrap-completeness defect and not a deeper
proof-boundary defect; fresh-process isolation is sound and the correction is
local to one module's admission surface; same-process runtime-object
attestation is NOT reopened
...
failed trusted-acquisition authority hash retained as non-authoritative =
...
IMPLEMENTATION COMPLETE / FAILED INDEPENDENT EXACT-HASH XHIGH REVIEW
...
POSTP1-003R1 review result =
FAIL — CORRECTED SUFFICIENCY GOVERNANCE INVALID

corrected sufficiency-governance definition hash retained as non-authoritative =
0ca7a2a8487e9c54b1b0f5ad07201ec39dfd20b328b5e09cb3b51b0de86c8242

failed sufficiency-governance definition hash retained as non-authoritative =
3f51c4d9d8f14689b3f6c863e1731a6ef170b9764162b79bd56cc369af4ae2c7

failed POSTP1-003 implementation commit =
90a0252744f333e2168ad3904efbc1ec14c5693e

failed POSTP1-003 review result =
...
PROSPECTIVE_CLOCK_INTEGRITY_V1
SOURCE_STREAM_EPOCH_V1
STREAM_LIVENESS_POLICY_V1
CVD_INTERVAL_COMPLETENESS_V1
...
- **Current implementation frontier:** Phase-1 implementation is complete and
  ...
  transitively replay the complete certified-parent warmup, PIT/source,
  universe, comparability, paired-risk and stop-lifecycle evidence graph. A
  strict evaluation-contract schema binds distinct candidate/control identities
  before authorization. Cutoffs are pure replay derivations: caller injection
  ...
  POSTP1-002V2A-R1 reproduced `b81c1702...b357af` but failed it because its
  scientific constructor still accepted caller-authored HTTP facts, endpoint
  ...
  response, and creates trusted provenance without accepting caller bytes,
  metadata or transport. Fixtures are explicitly non-authoritative and fail
  ...
  immutable envelope to PostgreSQL. POSTP1-002V2B reproduced its hash and all
  eight child bindings but found that registry and capability injection can
  replace the claimed production authority, appender success precedes durable
  commit, runtime executable identity is not enforced, and fresh deployment
  ...
  blocked. POSTP1-001V2B-R2 preserves that failed R1 directory and the accepted
  Ed25519 key exactly, adds parent-bound live material-value attestation and
  actual PostgreSQL session/privilege enforcement, and refreezes the final
  pre-data candidate at `bd55a3c0...02c4fc` with the same nine-child mechanical
  ...
  failed R2 candidate remains uncertified. POSTP1-001V2B-R3 preserves it
  unchanged and refreezes the same nine-child authority at
  `02f96203...1a12772`. The parent-bound PostgreSQL child and central live
  material snapshot now bind the collector role, schema, qualified table,
  ...
  hash. POSTP1-001V2A-I1 integrated that dependency at `b499c6a4...e584e076`,
  ...
  remain prohibited. POSTP1-002V2A-AD1-R1 reproduced the exact parent, all nine
  children and the current 11/11 owner census, but rejected the decision because
  the route audit proves only one acceptable path rather than every path. A
  mixed owner cycle with a documented alternate exit and a mixed documented /
  undocumented store-method owner both pass. Annotated variadic store consumers
  also evade discovery. The frozen candidate remains immutable, non-certified,
  unused and at zero observations. V2R1, POSTP1-003R3, POSTP1-004 and collection
  remain blocked.
  Historically,
...
- **Current IN_PROGRESS ticket:** None. POSTP1-001V2A-PAD4-R2 is
  **IMPLEMENTATION COMPLETE / FAILED INDEPENDENT EXACT-HASH FINAL xHIGH
  ...
  is documented as private and non-authoritative, but POSTP1-002V2A-PAD4-R2
  reproduced that scientific admission **can** use it and that a hand-built
  outcome needs no helper at all, which is the blocking finding;
...
- **Last completed review:** POSTP1-002V2A-PAD4-R2,
  **COMPLETE / FAIL — AUTHORITATIVE LAUNCH / ADMISSION BYPASS**, execution
  ...
  children reproduce and are parent-bound, and `restore_artifacts` reproduces
  the namespace byte-exact. The PAD4-R1 authority-ordering defect **is** closed
  on the authoritative path: all four bootstrap members, drifted plain and
  drifted fabricating, refuse before spawn with **0** subprocesses, **0**
  pycache allocations, markers **ABSENT** and nothing admitted. Fresh-exec
  isolation, Repair A, Repair B, Repair C and Repair D all independently
  reproduce as **VALID**, the compiled root witness, closed store grammar and
  exact eleven-owner graph are preserved, and the
  `candidate_review_authority_context` secondary candidate is **CONFIRMED NOT A
  DEFECT**. The blocking invariant is that scientific admission does not
  require, and cannot authenticate, the controller bootstrap pre-verification
  it attests to: `admit_worker_result` reads only `exit_status`,
  `request_digest`, `stdout` and `timed_out`, never
  `outcome.bootstrap_pre_verification`, and no mechanically closed call graph
  prevents a project-owned caller from combining the raw helper with admission.
  Three independent reproducers admit fabricated science — via the private
  helper, via a caller-fabricated PASS `bootstrap_pre_verification` mapping
  whose evidence then falsely attests `ALREADY_TRUSTED_CONTROLLER_CODE` and the
  exact trusted digest, and via a hand-built `WorkerProcessOutcome` with **no
  subprocess ever created**, which uses only public names. The failure is an
  **API/ADMISSION BYPASS**, local to one module's admission surface; it is not
  a bootstrap-completeness defect and not a deeper proof-boundary defect, and
...
- **Latest verified test baseline:** 5,202 passed, 3 skipped under
  ...
  corpus regression over all eighteen
  `etf`/`calendar`/`flow`/`trusted`/`corpus`/`prospective` test modules is
  1,518 passed with the three explained skips (the opt-in PostgreSQL runtime
  test and two composite component skips), against 1,521 collected. This
  documentation-only record corrects the previously stated wider figure of
  "1,478 passed over all seventeen modules", which POSTP1-002V2A-PAD4-R2
  independently re-measured as 1,518 passed over eighteen modules; the module
  census previously omitted
  `test_prospective_integration_evidence_sufficiency.py` (95 passed).
  Python 3.12.14 and `cryptography 50.0.1` were used on 2026-09-20, the wider
  re-measurement was taken on 2026-09-21, and every proof suite is
...
- **Preceding proof-architecture implementation validation:** POSTP1-001V2A-PAD4-R1
  ...
  bindings from the single builder registry, the exact CPython 3.12.14 identity
  shared by the corrected worker protocol and the compiled root witness, the
  frozen 116-module pre-I2 source fixture at `674b006a...3aadbb8` and the
  119-module candidate worker universe at `c6a71401...7aa89332`, the exact
  reviewed five-distribution third-party registry, the preserved 11-owner census
  and graph and the unchanged current-production blockers. The focused R1 suite
  passed 100 tests with real exec'd worker processes throughout. A side-by-side
  probe reproduced the PAD4 defect exactly — under `-I -S -B` alone a forged
  timestamp-valid `.pyc` executes `FORGED_BYTECODE_ID` — and showed the same
  tree returning `CERTIFIED` under the fresh `-X pycache_prefix` namespace; a
  real worker over a forged repository `__pycache__/flow.cpython-312.pyc` with
  byte-identical certified source returned `ETF_FLOW_5D`; a forged
  site-packages `alembic/__pycache__/__init__.cpython-312.pyc` never executed
  its marker; a pre-populated namespace refused before the certified protocol
  module was imported; a sourceless `flow/__init__.pyc` shadow refused on the
  exact origin check; pre-request drift of `flow.py`, both corrected worker
  modules and both reviewed PAD4 worker modules all refused while the request
  stayed bound to the frozen manifest; `0000...0000` and `deadbeef...deadbeef`
  were refused with the worker echoing them consistently; a `99.99.99-FORGED`
  version, a same-version different-artifact `RECORD` and a tampered installed
  `alembic/__init__.py` with unchanged `RECORD` all refused before the altered
  code executed; and the parent `_flow` feature-ID, generated
  `ScientificEtfFlowResult.__init__` and `sys.modules` mutations each left the
  worker result certified. Thirteen material mutations moved both the owning
  child hash and the parent hash, and the parent reproduced under three
  `PYTHONHASHSEED` values, reversed child order, an alternate working directory
  and a fresh output directory, with no operational cache path in any authority
  hash. `compileall` and `git diff --check` are clean on Python 3.12.14 with
  `cryptography 50.0.1`. No production behaviour changed, so the 5,202/3
  full-suite baseline remains current
- **Superseded proof-architecture implementation validation:** POSTP1-001V2A-PAD4
...
- **Latest proof-architecture validation:** POSTP1-002V2A-PAD4-R1
  independently reproduced the exact candidate parent
  `3415765f...fde37ebc` and all 20 material children mechanically enumerated
  from the single builder registry, with 20/20 child reproduction and 20/20
  parent binding. Fresh-exec isolation, Repair A bytecode binding and Repair D
  third-party semantic/content authority all reproduced **VALID**; Repair C
  trusted controller authority is **VALID FOR ITS STATED SCOPE** but cannot
  close bootstrap ordering, because the worker-visible authority fields are
  derivable from the supplied request; Repair B frozen source authority is
  **VALID FOR ORDINARY CERTIFIED SOURCE** and **INCOMPLETE FOR THE PRE-TRUST
  BOOTSTRAP SOURCES**. The review nevertheless failed on one blocking
  invariant: already-trusted code does not establish the certified identity of
  the worker bootstrap source before that source executes. The controller
  execs the entrypoint directly and the frozen expected manifest is compared
  inside the worker, so `etf_calendar_worker/entry_r1.py`,
  `etf_calendar_worker/__init__.py` and `etf_calendar_worker/protocol_r1.py`
  all run before the normal scientific verifier is trustworthy. A stable
  pre-launch entrypoint drift and a stable pre-launch protocol drift each left
  the execution marker **PRESENT** and the fabricated scientific result
  **ADMITTED**. The candidate stays immutable, non-certified, unused and at
  zero observations; no review fix was created because a review may not patch
  the authority ordering it is reviewing. This review record also corrects one
  documentation-only defect it identified, the combined
  PAD4-R1/PAD4/PAD3/PAD2/PAD1-R1/PAD1 architecture-suite count recorded here
  as 615 where the measured count is 617; no unrelated historical test count
  was altered and no wider or full rerun was required for the record itself,
  so the 5,202/3 full-suite baseline remains current
- **Preceding proof-architecture validation:** POSTP1-002V2A-PAD4 independently
...
- **Superseded proof-architecture validation:** POSTP1-002V2A-PAD2 independently
  ...
  current
- **Earlier proof-architecture validation:** POSTP1-002V2A-PAD1-R1 independently
  ...
  refusal. The focused R1 suite passed 81 tests; the combined R1/PAD1,
  original/R1/R2 architecture-decision and unchanged calendar regression set
  passed 337 tests. Artifact restoration, reversed child order, three fresh-
  process `PYTHONHASHSEED` values, alternate cwd/output, compileall and diff
  checks passed on Python 3.12.14. Independent TypeAlias and nested-definition-
...
- **Last relevant implementation/review commit:** POSTP1-001V2A-PAD4-R2
  ...
  scientific admission neither requires nor authenticates the controller
  bootstrap pre-verification it attests to. It overwrites no PAD4 or PAD4-R1
  artifact, changes no ETF calendar production code and makes no production
  isolated-worker authority available; its own namespace is now immutable and
  preserved. The POSTP1-002V2A-PAD4-R2 and POSTP1-002V2A-PAD4-R1
  review records are documentation-only; neither creates a review-fix commit,
  because the PAD4-R2 defect is an architecture correction that belongs to a
  successor candidate rather than a patch a review is permitted to apply here.
  POSTP1-001V2A-PAD4-R1 decision
  commit `37f0db2b12c86b5809d213ae3bdda77a46cc82c2` freezes the corrected pre-data candidate
  `3415765f...fde37ebc`, which failed POSTP1-002V2A-PAD4-R1, its independent
  exact-hash final xHigh proof-architecture review, because the worker
  bootstrap source is not bound before it executes. It overwrites no PAD4
  artifact and its own namespace is now immutable and preserved. POSTP1-001V2A-PAD4 decision
  ...
  fix commit. The failed AD1 decision
  ...
  incomplete runtime binding and missing role-provisioning boundaries.
  ...
  `3f51c4d9...f4ae2c7` remain explicit non-authoritative lineage. POSTP1-002R5 passed with review
...
[EPIC X](execution/post_phase1_prospective_integration_evidence_v1.md).
POSTP1-001, POSTP1-001R, POSTP1-001R2 and POSTP1-001R3 failed independent
review. POSTP1-001R4 corrects only the fourth review's four source-provenance/
completeness blockers and retains all failed hashes `aaa05c72...d37326`,
`0d4f1437...f45a9e`, `40e37067...c9862` and `e60a9514...a7dca` as explicit
lineage. It
does not repair BTC-019 and is not `BTC_REFERENCE_COMPOSITE_V6`: no frozen V3, V4,
...
that now includes `raw.liquidations` and mechanically binds all 33
`INITIAL_FEATURE_NAMES` to exactly one raw capture contract or deterministic
owner; a rebuilt machine-derived warmup table that keeps each owner's eligible
trailing window, minimum qualifying observation count and evaluability
predicate as three separate frozen fields; two deterministic portfolio tracks from one frozen flat
...
hand-selected. Isolated-venue events require one observed touch and a full
confirmation quorum of observed non-touches. Gap-through uses the immediately
preceding contiguous hourly required-provider consensus close and refuses a gap
greater than one hour. Stop universes use the control track's active stop, so a
candidate cannot shrink its own denominator. Every universe predicate reads inputs and per-track state only,
so no timestamp can enter or leave a denominator because the two tracks
agreed. Trade action is bound to a decision-producing owner chain and ordered
action envelope; trade eligibility is bound to the full new-entry permission
...
`REQUIRES_NEW_COLLECTOR_IN_FIRST_COLLECTION_TICKET`. A
second is why the
...
POSTP1-001R2 separated Phase-1 **feature semantics** from
**prospective acquisition semantics** and freezes the three contracts Phase-1
never owned, each declared explicitly as new pre-data governance with
`historically_inherited = false` and its own bound definition hash.
`PROSPECTIVE_CVD_ACQUISITION_V1` records that `spot_perp_cvd_spread` specifies
no cadence at all — it accepts arbitrary common timestamps and requires 20
prior common observations — and selects `1h` prospectively against eight
predeclared criteria with `4h` and `1d` assessed and rejected on record.
POSTP1-001R3 first bound that cadence to Kraken spot WebSocket v2 `BTC/USD` and
Kraken Futures WebSocket v1 `PI_XBTUSD`, retains provider/instrument/revision
and completion evidence, and requires the current plus 20 immediately prior
contiguous UTC hours before the historical observation-count owner.
`PROSPECTIVE_BTC_MARKET_CAP_ACQUISITION_V1` records
`existing_market_cap_producer = NONE` and freezes one exact
`BTC_MARKET_CAP_USD` / `market_cap` / `usd` series from provider `coingecko` on
`raw.generic_series` at a daily UTC grid, so `OI_INTENSITY` no longer resolves
its input by searching an unqualified generic-series family; a price-times-supply
construction is rejected because it would make an exogenous input depend on the
reference under test. POSTP1-001R3 first made that source operationally exact: fixed
00:45/00:50/00:55 UTC polling records local response completion, requeries
day-1 through day-3, requires the exact scheduled date with no older fallback,
and supplies only one latest available revision per timestamp to the historical
owner. `PROSPECTIVE_LIQUIDATION_CAPTURE_V1` records that
`aggregate_btc_derivatives_available_at` collapses a missing feed and an
observed zero-event feed to the same numeric zero, and persists `feed_status`,
`event_count` and both notionals independently in a new append-only feed-state
table; `SOURCE_UNAVAILABLE`, `LATE` and `INVALID` make the required input
missing, while `OBSERVED_ZERO_EVENTS` legitimately carries zero notional.
POSTP1-001R3 first bound liquidation capture to liquidation-typed events on Kraken
Futures `PI_XBTUSD`; affirmative zero requires pre-start acknowledgement,
continuous WebSocket and heartbeat coverage through close, no sequence gap or
invalid/conflicting event, a consistent unique-UID census and timely local
finalization.
`PROSPECTIVE_LIQUIDATION_PERCENTILE_ADAPTER_V1` supplies the
`liquidation_percentile` normalization no executable owner produced, inheriting
the repository's single midrank prior-window convention and the 730-day / 365
minimum-observation window of the `volatility_percentile` owner already
co-consumed by `calculate_orderliness_score`. The false 750-day warmup is
withdrawn: `VOL_PERCENTILE_2Y` keeps a 730-day eligible trailing window, needs
365 prior `RV_20` observations inside it plus 21 contiguous daily closes per
`RV_20`, and therefore first defines at 386 contiguous daily sessions — 385
elapsed calendar days — pinned against the production owner by a synthetic
fixture. Evaluability is the owner's rule, never `elapsed_days >=` a constant.
POSTP1-002R2 independently recomputed the top hash and all fifteen children,
confirmed zero V2 gate changes, and retained the corrected warmup and
missing-versus-zero semantics. It nevertheless rejected this hash as
`FAIL — MARKET CAP SOURCE CONTRACT INVALID` /
`PROSPECTIVE_PROTOCOL_REQUIRES_FIX`: CoinGecko's 00:00 UTC daily point is
documented as unavailable until 00:35 on the following UTC day, after the
frozen 00:05 decision; the contract also cannot recover exact publication and
restatement instants from the provider payload. The CVD contract additionally
defers its actual providers and perpetual universe and allows its count-based
owner to bridge an interior missing hour, while the liquidation contract has
no frozen provider/instrument coverage census. These are pre-data acquisition
decisions, not mechanical review fixes. POSTP1-001R3 corrected those findings
and refroze them at `e60a9514...a7dca`, but POSTP1-002R3 rejected that exact
hash because CoinGecko request/response identity, Kraken stream/hour/day
completeness and local-clock integrity were not mechanically reproducible.
POSTP1-001R4 now binds exact `YYYY-MM-DD` CoinGecko request/response provenance,
...
and epoch-freeze enforcement. POSTP1-003R1 refroze governance at
`0ca7a2a8...e86c8242`, but its repeat independent exact-hash xHigh review
failed: stop moves can manufacture distinct units inside one position, blind
source summaries are not transitively replayed, unverified cutoff records can
be frozen and evaluated, and an empty hashed object can authorize the
evaluation epoch. POSTP1-003R2 closes those bounded findings at
...
- EPIC X's source-replayability correction at `8915d991...fbfac7d7` resolves the
  ...
  authority or durably freeze an evaluated epoch. POSTP1-003R1 addressed those
  findings at `0ca7a2a8...e86c8242`: raw and natural-unit minima are 381 for the
  exact-1.0 cross-market gate and 93 for nearest-rank p95, per-metric replay
  coverage is exactly floored at 0.99, and the intended blind/cutoff contracts
  are hash-bound. Its repeat review nevertheless found that active-stop changes
  inflate one position into multiple dependence units, the blind source summary
  can be self-rehashed without certified parent replay, the registry can freeze
...
- The eight prospective Stage-B measurements have a new pre-data sufficiency-
  governance review candidate at `0c0c0f96...5863c64e`; it is not collection
  authority until final independent exact-hash review passes. The exact 1.0
  performance threshold remains unchanged and separately requires 381 raw and
  distinct natural evidence units under the common 95% / 1%-error reference;
  nearest-rank risk p95 requires 93 raw and distinct sizing units under its
  repeated-tail rule. All eight metrics additionally require at least 0.99
  authoritative replay coverage and complete scheduled-slot accounting.
  POSTP1-003R2 makes one root control-position lifecycle the stop dependence
  unit, replays all blind facts from exact certified-parent manifests, strictly
  identifies evaluation contracts, and independently derives and revalidates
  the earliest frozen cutoff and terminal result. POSTP1-004 remains blocked
  through this candidate's final review and the later POSTP1-004 implementation
  review.
