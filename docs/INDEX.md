...
| [`prospective_evidence/etf_calendar_runtime_owner_attestation_v1/`](../prospective_evidence/etf_calendar_runtime_owner_attestation_v1/) | Failed hash-bound `ETF_CALENDAR_RUNTIME_OWNER_ATTESTATION_V1` proof architecture: the preserved compiled root-binding witness and closed AST store-use grammar plus measured pre/post runtime attestation of the eleven frozen replay owners and their purported transitive execution closure | Immutable pre-data candidate at `b8f8b92d...f6b5b996` that failed POSTP1-002V2A-PAD3 because project-owned import attestation stops at module source and top-level function identity instead of recursively attesting mutable runtime values, classes and dependencies read by those functions; generated dataclass methods are also not fingerprinted; non-certified, unused, and authorizes no calendar implementation, V2 correction or collection |
