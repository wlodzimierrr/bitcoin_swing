...
| [`prospective_evidence/etf_calendar_store_capability_normal_form_v1/`](../prospective_evidence/etf_calendar_store_capability_normal_form_v1/) | Hash-bound `ETF_CALENDAR_STORE_CAPABILITY_NORMAL_FORM_V1` closed proof architecture | Frozen pre-data proof-architecture decision at `9f6af179...7ac86295`; awaiting independent exact-hash xHigh review; defines a closed allowed store-use grammar and rejects all unsupported store-bearing uses; authorizes no calendar implementation, V2 correction, or collection |
